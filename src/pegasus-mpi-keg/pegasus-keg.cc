...
//orc@22-06: adding Decaf headers
#ifdef WITH_DECAF
...
fprintf(stderr, "lin631 input_file size is %lu\n",input_files_size);
...
main( int argc, char *argv[] )
{
    ...
    fprintf(stderr,"Halo from Decaf at rank %d\n", rank);
    ...
    for ( int i = 1; i < argc; ++i )
    {
        ...
        if ( state >= 10 )
        {
            ...
            case 18:
                prod = strtoul(s, 0, 10);
                if (prod == 1)
			fprintf(stderr, "I'm Decaf producer at rank %d\n", decaf->world->rank());
                else if (prod == 2) //orc@25-06: adding this intermediate nodes for the pipelined scenario
			fprintf(stderr, "I'm Decaf producer and consumer - like a dataflow at rank %d\n", decaf->world->rank());
                else
...
#ifdef WITH_DECAF
	unsigned long data_size = mem_buf_size;
#endif
        ...
        if(prod==0)
        {
            //orc@23-06: receiving this memory buffer at the consumer
            map<string, pConstructData> in_data;
            while (decaf->get(in_data))
            {
                ArrayFieldc field = in_data.at("in")->getFieldData<ArrayFieldc>("pos");
                if (field){
                    fprintf(stderr, "Received data in con\n");
		    char* con_buffer = field.getArray();
                }
                else
        ...
        if(prod==2) //orc@25-06: intermediate tasks in the pipeline
        {
            ...
            while (decaf->get(in_data))
            {
                ...
                if (field){
                    ...
                    fprintf(stderr, "Error: null pointer in dataflow\n");

                container->appendData("pos", field,
                                      DECAF_NOFLAG, DECAF_PRIVATE,
                                      DECAF_SPLIT_DEFAULT, DECAF_MERGE_DEFAULT);
                decaf->put(container, "out");
                if (sleeptime) sleep(sleeptime);
            ...
            // terminate the task (mandatory) by sending a quit message to the rest of the workflow
            fprintf(stderr, "intermediate task %d terminating\n", decaf->world->rank());
            decaf->terminate();
